 ...
 * 本文件中的 `${...}` 出现在普通字符串字面量里，是 CodeMirror 补全 snippet 的
 * 占位符语法（由 @codemirror/autocomplete 的 snippet 机制在编辑器内解析为
 * 光标跳位点），并非 JavaScript 模板字符串插值。它们必须以字面文本形式保留，
 * 若按该规则的建议改为模板字符串反而会破坏补全功能，故整文件关闭此规则。
 */
...
const COMMON_DEFINITIONS: TemplateCompletionDefinition[] = [
    ...
    {
        label: 'lunar_year_ganzhi',
        detail: '干支纪年，需要安装变量扩展',
        type: 'variable'
    },
    {
        label: 'current_holiday',
        detail: '当前节日，需要安装变量扩展',
        type: 'variable'
    },
    {
        label: 'random',
    ...
    },
    {
        ...
        detail: '按会话稳定地选取一项',
        type: 'function',
        snippet: "pick('${option1}', '${option2}')}"
    },
    {
        label: 'timeDiff',
        detail: '计算两个时间戳之间的间隔',
        type: 'function',
        snippet: "timeDiff('${from}', '${to}')}"
    },
    {
        label: 'latest_message',
        detail: '最近的群聊消息，需要安装变量扩展',
        type: 'function',
        snippet: "latest_message('${count}')}"
    }
]
...
const CONTEXT_DEFINITIONS: TemplateCompletionDefinition[] = [
    ...
    },
    {
        ...
        detail: '角色当前状态',
        type: 'variable',
        contexts: ['character-input', 'character-system', 'character-preset']
    },
    {
        label: 'stickers',
        detail: '表情包占位内容',
    ...
    {
        label: 'living_memory',
        detail: '角色记忆与用户画像，需要安装长期记忆扩展',
        type: 'function',
        contexts: ['character-input', 'character-system', 'character-preset']
    },
    {
        label: 'memesluna',
        detail: '表情包提示词，需要安装表情包扩展',
        type: 'variable'
    },
    {
        label: 'endpoint',
        detail: '表情包合集与端点路由清单，需要安装表情包扩展',
        type: 'variable'
    },
    {
        label: 'sender',
...
function parseControlTag(content: string): ControlTagResult {
    ...
    for (const name of ['if', 'while', 'repeat'] as const) {
        ...
        if (content.startsWith(prefix)) {
            const expression = parseTemplateExpression(
